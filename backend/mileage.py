...
def _extract_visits(events: list[dict]) -> list[dict]:
    ...
    prompt = (
        ...
        "- label: the containing event's subject/title, VERBATIM (it becomes the "
        "business-purpose field on mileage reports). Only if the subject is blank, "
        "a short label from the body (e.g. 'Hernandez flooring install').\n"
